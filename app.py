...
def create_app():
    ...
    # r = redis.Redis(host='localhost', port=6379, db=0)  # use for local redis
    app.config['SESSION_TYPE'] = 'filesystem'  # set to 'redis' or 'filesystem'
    app.jinja_env.auto_reload = True
    app.config['TEMPLATES_AUTO_RELOAD'] = True
    '''For local w/out redis, session, set variables and use 'nonlocal' on each page:
